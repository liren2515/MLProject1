...
print('Run Least Square Stochastic Gradient Descent!')
...
save_folder = 'results/least_square_SGD' if not config.data_balance else 'results/least_square_SGD_balance'
...
# Save results
if config.augmentation:
    csv_save_path = os.path.join(save_folder, 'least_square_SGD_degree_%d.csv'%config.power_degree)
else:
    csv_save_path = os.path.join(save_folder, 'least_square_SGD.csv')
